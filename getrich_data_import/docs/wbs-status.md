...
| T2.3 | Done | `20_market.sql` creates index/future/option daily and minute fact tables. |
| T2.4 | Partial | Compression policies and continuous aggregates are defined for 1m tables. |
...
| T3.5 | Partial | TradingCalendarService supports open-day checks, previous/next trading day, and configurable night-session assignment. Pipeline integration is pending. |
...
Local `getrich` PostgreSQL/TimescaleDB was initialized through `migrate-schema`.

- PostgreSQL server version: 16.14
- TimescaleDB version: 2.27.2
- TimescaleDB license: apache
- All expected tables, hypertables, and migration records pass `verify-schema`
- Compression, retention policy, and continuous aggregate DDL degrade gracefully under Apache license; regular materialized views are created for 1m-to-1d aggregates.
