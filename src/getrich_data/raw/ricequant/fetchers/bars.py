...
from getrich_data.common.retry import chunk_list, int_to_date, retry_call, sleep_s, today_int
from getrich_data.raw.base import BaseFetcher
from getrich_data.raw.ricequant.client import RQ_TYPE_MAP
...
class InstrumentsFetcher(BaseFetcher):
    ...
    def fetch(self, mode: str = "update") -> int:
        ...
        for asset in ASSETS:
            ...
            df = retry_call(
                self.client.all_instruments, rq_type, self.client.market,
...
class CalendarFetcher(BaseFetcher):
    ...
    def fetch(self, mode: str = "update") -> int:
        ...
        dates = retry_call(
            self.client.get_trading_dates, start, end, self.client.market,
            max_retries=self.ctx.max_retries, backoff_base=self.ctx.backoff_base,
            logger=self.log,
        )
    ...
    def fetch(self, mode: str = "update") -> int:
        ...
        for asset in ASSETS:
            ...
            for chunk in chunk_list(codes, self.CODE_CHUNK_SIZE):
                df = retry_call(
                    self.client.get_price, chunk, start, end, "1d", "none", self.client.market,
                    max_retries=self.ctx.max_retries, backoff_base=self.ctx.backoff_base,
                    logger=self.log,
                )
                ...
                for code, grp in reset.groupby(key):
                    out = self.paths.dataset_file(self.PROVIDER, f"{self.DATASET}/{asset}", str(code))
