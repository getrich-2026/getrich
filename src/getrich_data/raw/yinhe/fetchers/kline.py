...
from getrich_data.common.parquet import (
    last_index_date,
    read_parquet_if_exists,
    write_parquet,
)
from getrich_data.common.retry import (
    chunk_date_range,
    chunk_list,
    next_int_date,
    retry_call,
...
from getrich_data.raw.yinhe.fetchers.reference import INIT_START_DATE, SECURITY_TYPES
    ...
    def fetch(self, mode: str = "update") -> int:
        ...
        for chunk in chunk_list(codes, self.CODE_CHUNK_SIZE):
            ...
            for begin, stop in chunk_date_range(floor, end, self.DATE_CHUNK_DAYS):
                result = retry_call(
                    self.client.query_kline, todo, begin, stop, self.PERIOD,
