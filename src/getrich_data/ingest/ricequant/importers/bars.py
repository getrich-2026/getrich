...
from getrich_data.ingest.ricequant.adapter import ASSETS, RicequantAdapter, split_obid
...
class _BarsImporter(BaseImporter):
    ...
    def _normalize(self, raw: pd.DataFrame, instrument_id: int) -> pd.DataFrame:
        time_col = "date" if "date" in raw.columns else ("datetime" if "datetime" in raw.columns else raw.columns[0])
