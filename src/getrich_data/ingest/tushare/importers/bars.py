...
from getrich_data.ingest.tushare.adapter import TushareAdapter
from getrich_data.ingest.tushare.importers.reference import _require, parse_date
...
class StockBars1dImporter(_BaseBarsImporter):
    ...
    def _merge_limits(self, market: pd.DataFrame, adapter: TushareAdapter) -> pd.DataFrame:
        ...
        merged = market.merge(lim[keep], on=["ts_code", "trade_date"], how="left", validate="one_to_one")

        _to_numeric(merged, ["up_limit", "down_limit"])
        bad = (merged["up_limit"] <= 0) | (merged["down_limit"] <= 0) | (
    ...
    def _trading_status(self, market: pd.DataFrame, adapter: TushareAdapter) -> pd.Series:
        ...
        return pd.Series(
            [
                "HALTED" if (k in halted and k not in resumed) else "NORMAL"
                for k in keys
            ],
            index=market.index,
            dtype=object,
