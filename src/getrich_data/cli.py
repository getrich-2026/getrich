...
用法::
    ...
    getrich stream <provider> [--symbols ...]          # 长驻，需真实 SDK
    getrich db migrate | status
    getrich own list | set <target> <provider> <channel> | release <target>

provider ∈ {yinhe, ricequant, insight}（stream 仅 yinhe/insight）。
"""
...
from getrich_data.common.config import Config, load_config
from getrich_data.common.db.pool import PgConfig, connect
from getrich_data.common.logging import configure_logging, get_logger
...
def cmd_raw(args: argparse.Namespace, cfg: Config) -> int:
    from getrich_data.raw import run_provider
...
def cmd_ingest(args: argparse.Namespace, cfg: Config) -> int:
    from getrich_data.ingest import run_provider

    with connect(_pg(cfg)) as conn:
        results = run_provider(
            args.provider, conn, cfg,
            only=_split(args.only), force_ownership=args.force_ownership,
        )
    for r in results:
        log.info("ingest %s -> rows=%d warnings=%d", r.target, r.rows_written, len(r.warnings))
    print(f"ingest {args.provider} 完成: " + ", ".join(f"{r.target}={r.rows_written}" for r in results))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="getrich", description="GetRich 数据接入层 CLI")
    p.add_argument("--config", help="配置文件路径", default=None)
    sub = p.add_subparsers(dest="cmd", required=True)

    raw = sub.add_parser("raw", help="抓取 raw 数据落 parquet")
    raw.add_argument("provider", choices=["yinhe", "ricequant", "insight"])
    ...
    ing.add_argument("provider", choices=["yinhe", "ricequant", "insight"])
