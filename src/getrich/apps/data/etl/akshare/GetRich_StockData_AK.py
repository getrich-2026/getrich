...
def get_stock_info(symbol: str) -> pd.DataFrame:
    ...
    :return: 个股信息
    """
    stock_info_df: pd.DataFrame = ak.stock_individual_basic_info_xq(symbol=symbol)
...
def get_stock_quote(symbol: str) -> pd.DataFrame:
    ...
    :return: 个股行情报价
    """
    stock_quote_df: pd.DataFrame = ak.stock_bid_ask_em(symbol=symbol)
    return stock_quote_df


# 3. 实时行情数据-东方财富
def get_stock_realtime_quotes(dataapi: str = "em") -> pd.DataFrame:
    """
    获取个股实时行情数据
...
# 4. 沪市A股实时行情数据-东方财富
def get_stock_sh_realtime_quotes() -> pd.DataFrame:
    """
    获取沪市A股实时行情数据
    :return: 沪市A股实时行情数据
    """
    stock_sh_realtime_quotes_df: pd.DataFrame = ak.stock_sh_a_spot_em()
    return stock_sh_realtime_quotes_df


# 5. 深市A股实时行情数据-东方财富
def get_stock_sz_realtime_quotes() -> pd.DataFrame:
    """
    获取深市A股实时行情数据
    :return: 深市A股实时行情数据
    """
    stock_sz_realtime_quotes_df: pd.DataFrame = ak.stock_sz_a_spot_em()
    return stock_sz_realtime_quotes_df


# 6. 北交所实时行情数据-东方财富
def get_stock_bj_realtime_quotes() -> pd.DataFrame:
    """
    获取北交所实时行情数据
    :return: 北交所实时行情数据
    """
    stock_bj_realtime_quotes_df: pd.DataFrame = ak.stock_bj_a_spot_em()
    return stock_bj_realtime_quotes_df


# 7. 新股实时行情数据-东方财富
def get_stock_new_realtime_quotes() -> pd.DataFrame:
    """
    获取新股实时行情数据
    :return: 新股实时行情数据
    """
    stock_new_realtime_quotes_df: pd.DataFrame = ak.stock_new_a_spot_em()
    return stock_new_realtime_quotes_df


# 8. 创业板板实时行情数据-东方财富
def get_stock_cyb_realtime_quotes() -> pd.DataFrame:
    """
    获取创业板板实时行情数据
    :return: 创业板板实时行情数据
    """
    stock_cyb_realtime_quotes_df: pd.DataFrame = ak.stock_cy_a_spot_em()
    return stock_cyb_realtime_quotes_df


# 9. 科创板实时行情数据-东方财富
def get_stock_kcb_realtime_quotes() -> pd.DataFrame:
    """
    获取科创板实时行情数据
    :return: 科创板实时行情数据
    """
    stock_kcb_realtime_quotes_df: pd.DataFrame = ak.stock_kc_a_spot_em()
    return stock_kcb_realtime_quotes_df


# 10. 个股实时行情数据-雪球
def get_stock_xq_realtime_quotes(symbol: str) -> pd.DataFrame:
    """
    获取个股实时行情数据
    :param symbol: symbol="SH600000"; 证券代码，可以是 A 股个股代码，A 股场内基金代码，A 股指数，美股代码, 美股指数
    :return: 个股实时行情数据
    """
    df: pd.DataFrame = ak.stock_individual_spot_xq(symbol=symbol)
    return df


# 11. 个股历史行情数据-东财
def get_stock_history_em(
...
# 14. 当前交易日的个股分钟数据-新浪
def get_stock_minute_sina(
    symbol: str | None = None, period: str = "1", adjust: str = ""
) -> pd.DataFrame:
    """
    获取个股分钟数据
    :param symbol: symbol='sh600000';  股票代码可以在 get_stock_realtime_quotes(dataapi='sina')获取
    ...
    :return: 个股分钟数据
    """
    df: pd.DataFrame = ak.stock_zh_a_minute(symbol=symbol, period=period, adjust=adjust)
    return df


# 15. 获取历史的个股分钟数据-东财
def get_stock_minute_em(
    symbol: str | None = None,
    start_date: str | None = None,
    end_date: str | None = None,
    period: str = "1",
...
# 16. 日内分时个股行情数据-东财
def get_stock_intraday_em(symbol: str) -> pd.DataFrame:
    """
    获取日内分时个股行情数据
    :param symbol: symbol='000001';  股票代码可以在 get_stock_realtime_quotes(dataapi='em')获取
    :return: 日内分时个股行情数据
    """
    df: pd.DataFrame = ak.stock_intraday_em(symbol=symbol)
    return df


# 17. 日内分时个股行情数据-新浪
def get_stock_intraday_sina(symbol: str, date: str) -> pd.DataFrame:
    """
    获取日内分时个股行情数据
    :param symbol: symbol='sh600000';  股票代码可以在 get_stock_realtime_quotes(dataapi='sina')获取
    ...
    """
    df: pd.DataFrame = ak.stock_intraday_sina(symbol=symbol, date=date)
    return df


# 18. 股票历史的盘前数据-东财
def get_stock_pre_min_em(
    symbol: str | None = None,
    ...
    """
    df: pd.DataFrame = ak.stock_zh_a_hist_pre_min_em(
...
# 19. 股票历史分笔数据-腾讯
def get_stock_tick_tencent(symbol: str) -> pd.DataFrame:
    """
    获取股票历史分笔数据
    :param symbol: symbol='sh600000';
    ...
    """
    df: pd.DataFrame = ak.stock_zh_a_tick_tx_js(symbol=symbol)
    return df


# 20. 获取新浪每天最新的次新股最近交易日数据
def get_stock_new_stock_sina() -> pd.DataFrame:
    """
    获取新浪每天最新的次新股最近交易日数据
    :return: 新股数据
    symbol object 新浪代码
    ...
    """
    df: pd.DataFrame = ak.stock_zh_a_new()
    return df


# 21. 获取A股指定交易日所有上市公司的新闻动态-东财
def get_stock_news_em(date: str) -> pd.DataFrame:
    """
    获取A股指定交易日所有上市公司的新闻动态
    :param date: date='20210809'; 日期
    :return: A股指定交易日所有上市公司的新闻动态
    """
    df: pd.DataFrame = ak.stock_gsrl_gsdt_em(date=date)
    return df


# 22. 获取A股新股数据当前交易日的行情数据-东财
def get_stock_new_stock_em() -> pd.DataFrame:
    """
    获取A股新股数据当前交易日的行情数据
    :return: A股新股数据当前交易日的行情数据
    """
    df: pd.DataFrame = ak.stock_zh_a_new_em()
    return df


# 23. 科创板实时行情数据-新浪
def get_stock_kcb_realtime_quotes_sina() -> pd.DataFrame:
    """
    获取科创板实时行情数据
    :return: 科创板实时行情数据
    """
    df: pd.DataFrame = ak.stock_zh_kcb_spot()
    return df


# 24. 科创板历史行情数据-新浪
def get_stock_kcb_history_sina(symbol: str, adjust: str = "") -> pd.DataFrame:
    """
    获取科创板历史行情数据
    ...
    """
    df: pd.DataFrame = ak.stock_zh_kcb_daily(symbol=symbol, adjust=adjust)
    return df


# 25. 科创板公告-东财
def get_stock_kcb_report_em(from_page: int, to_page: int) -> pd.DataFrame:
    """
    获取科创板公告
    ...
    """
    df: pd.DataFrame = ak.stock_zh_kcb_report_em(from_page=from_page, to_page=to_page)
    return df


# 26. AH股实时行情数据-东财
def get_ah_realtime_spot_em() -> pd.DataFrame:
    """
    获取AH股实时行情数据
    :return: AH股实时行情数据

    """
    df: pd.DataFrame = ak.stock_zh_ah_spot_em()
    return df


# 27. 获取AH股的历史行情数据-腾讯
def get_ah_history_tencent(
    symbol: str, start_year: str, end_year: str, adjust: str = ""
) -> pd.DataFrame:
    """
    获取AH股的历史行情数据
    ...
    :return: AH股的历史行情数据
    """
    df: pd.DataFrame = ak.stock_zh_ah_daily(
        symbol=symbol, start_year=start_year, end_year=end_year, adjust=adjust
    )
    return df


# 28. 获取AH股名单-腾讯
def get_stock_ah_name_tencent() -> pd.DataFrame:
    """
    获取AH股名单
    :return: AH股名单
    """
    df: pd.DataFrame = ak.stock_zh_ah_name()
    return df


# 29. 美股实时行情数据-东财
def get_stock_us_realtime_quotes() -> pd.DataFrame:
    """
    获取美股实时行情数据
...
# 30. 获取美股实时行情数据-新浪
def get_stock_us_realtime_quotes_sina() -> pd.DataFrame:
    """
    获取美股实时行情数据
    :return: 美股所有上市公司的实时行情数据，有延迟15分钟
    """
    df: pd.DataFrame = ak.stock_us_spot()
    return df
...
def get_stock_us_history(
    ...
    :return: 美股历史数据
    """
    df: pd.DataFrame = ak.stock_us_hist(
...
# 32. 获取港股实时行情数据-东财
def get_stock_hk_realtime_quotes() -> pd.DataFrame:
    """
    获取港股实时行情数据
    :return: 港股实时行情数据
    """
...
# 33. 获取港股主板实时行情数据-东财
def get_stock_hk_main_realtime_quotes() -> pd.DataFrame:
    """
    获取港股主板实时行情数据
    :return: 港股主板实时行情数据
    """
    df: pd.DataFrame = ak.stock_hk_main_board_spot_em()
    return df


# 34.获取港股分时数据-东财
def get_stock_hk_minute_em(
    symbol: str, period: str, start_date: str, end_date: str, adjust: str = ""
) -> pd.DataFrame:
    """
    获取港股分时数据
    ...
    :return: 港股分时数据
    """
    df: pd.DataFrame = ak.stock_hk_hist_min_em(
        symbol=symbol, period=period, start_date=start_date, end_date=end_date, adjust=adjust
    )
    return df
...
# 36. 获取知名港股实时数据-东财
def get_stock_hk_famous_spot_em() -> pd.DataFrame:
    """
    获取知名港股实时数据
    :return: 港股实时行情数据
    """
    df: pd.DataFrame = ak.stock_hk_famous_spot_em()
    return df


# 37. 获取知名美股实时数据-东财
def get_stock_us_famous_spot_em() -> pd.DataFrame:
    """
    获取知名美股实时数据
    :return: 美股实时行情数据
    """
    df: pd.DataFrame = ak.stock_us_famous_spot_em()
    return df


# 38. A股机构调研统计-东财
def get_stock_jgdy_em(date: str) -> pd.DataFrame:
    """
    获取A股机构调研统计
    :param date: date="20250729"; 日期
    :return: A股机构调研统计
    """
    df: pd.DataFrame = ak.stock_jgdy_tj_em(date=date)
    return df


# 39. 股票质押市场概况
def get_stock_pledge_em() -> pd.DataFrame:
    """
    获取股票质押市场概况
    :return: 股票质押市场概况
    """
    df: pd.DataFrame = ak.stock_gpzy_profile_em()
    return df


# 40. 上市公司质押比例
def get_stock_pledge_ratio_em(date: str) -> pd.DataFrame:
    """
    获取上市公司质押比例
    ...
    :return: 上市公司质押比例
    """
    df: pd.DataFrame = ak.stock_gpzy_pledge_ratio_em(date=date)
    return df


# 41. 重要股东股份质押明细
def get_stock_pledge_detail_em() -> pd.DataFrame:
    """
    获取重要股东股份质押明细
    ...
    :return: 重要股东股份质押明细
    """
    df: pd.DataFrame = ak.stock_gpzy_pledge_ratio_detail_em()
    return df


# 42. 质押机构分布统计-证券公司
def get_stock_pledge_company_em() -> pd.DataFrame:
    """
    获取质押机构分布统计-证券公司
    :return: 质押机构分布统计-证券公司
    """
    df: pd.DataFrame = ak.stock_gpzy_distribute_statistics_company_em()
    return df


# 43. 质押机构分布统计-银行
def get_stock_pledge_bank_em() -> pd.DataFrame:
    """
    获取质押机构分布统计-银行
    :return: 质押机构分布统计-银行
    """
    df: pd.DataFrame = ak.stock_gpzy_distribute_statistics_bank_em()
    return df


# 44. 行业质押比例
def get_industry_pledge_ratio_em() -> pd.DataFrame:
    """
    获取行业质押比例
    :return: 行业质押比例
    """
    df: pd.DataFrame = ak.stock_gpzy_industry_data_em()
    return df


# 45. 股票账户月度统计
def get_stock_account_month_em() -> pd.DataFrame:
    """
    获取股票账户月度统计
    :return: 股票账户月度统计
    """
    df: pd.DataFrame = ak.stock_account_statistics_em()
    return df


# 46. 分析师指数排行
def get_stock_analyst_index_em(year: str) -> pd.DataFrame:
    """
    获取分析师指数排行
    :param year: year="2025"; 2025年
    :return: 分析师指数排行
    """
    df: pd.DataFrame = ak.stock_analyst_rank_em(year=year)
    return df


# 47. 分析师预测详情
def get_stock_analyst_detail_em(analyst_id: str, indicator: str) -> pd.DataFrame:
    """
    获取分析师预测详情
    ...
    :return: 分析师预测详情
    """
    df: pd.DataFrame = ak.stock_analyst_detail_em(analyst_id=analyst_id, indicator=indicator)
    return df


# 48. 股票评论热度
def get_stock_comment_hot_em() -> pd.DataFrame:
    """
    获取股票评论热度
    :return: 股票评论热度
    """
    df: pd.DataFrame = ak.stock_comment_em()
    return df


# 49. 股票评论热度-个股
def get_stock_comment_hot_detail_em(symbol: str) -> pd.DataFrame:
    """
    获取股票评论热度-个股
    :param symbol: symbol="600000"; 股票代码
    :return: 股票评论热度-个股
    """
    df: pd.DataFrame = ak.stock_comment_detail_zlkp_jgcyd_em(symbol=symbol)
    return df


# 50. 股票-东财用户关注度
def get_stock_user_attention_em(symbol: str) -> pd.DataFrame:
    """
    获取股票-东财用户关注度
    :param symbol: symbol="600000"; 股票代码
    :return: 股票-东财用户关注度
    """
    df: pd.DataFrame = ak.stock_comment_detail_scrd_focus_em(symbol=symbol)
    return df


# 51. 沪深港股通资金流向
def get_stock_hsgt_em() -> pd.DataFrame:
    """
    获取沪深港股通资金流向
    :return: 沪深港股通资金流向
    交易日	object	-
...
# 52. 沪深港股通历史数据
def get_stock_hsgt_history_em(symbol: str) -> pd.DataFrame:
    """
    获取沪深港股通历史数据
    ...
    领涨股-代码	object	-
    """
    df: pd.DataFrame = ak.stock_hsgt_hist_em(symbol=symbol)
    return df


# 53. 业绩报表
def get_stock_performance_em(date: str) -> pd.DataFrame:
    """
    获取业绩报表
    date:date="20200331"; choice of {"XXXX0331", "XXXX0630", "XXXX0930", "XXXX1231"}; 从 20100331 开始
    :return: 业绩报表
    """
    df: pd.DataFrame = ak.stock_yjbb_em(date=date)
    return df


# 54. 业绩快报
def get_stock_performance_fast_em(date: str) -> pd.DataFrame:
    """
    获取业绩快报
    date:date="20200331"; choice of {"XXXX0331", "XXXX0630", "XXXX0930", "XXXX1231"}; 从 20100331 开始
    :return: 业绩快报
    """
    df: pd.DataFrame = ak.stock_yjkb_em(date=date)
    return df


# 55. 业绩预告
def get_stock_performance_forecast_em(date: str) -> pd.DataFrame:
    """
    获取业绩预告
    date:date="20200331"; choice of {"XXXX0331", "XXXX0630", "XXXX0930", "XXXX1231"}; 从 20100331 开始
    :return: 业绩预告
    """
    df: pd.DataFrame = ak.stock_yjyg_em(date=date)
    return df


# 56. 资产负债表
def get_stock_balance_sheet_em(date: str) -> pd.DataFrame:
    """
    获取资产负债表
    date:date="20200331"; choice of {"XXXX0331", "XXXX0630", "XXXX0930", "XXXX1231"}; 从 20100331 开始
    :return: 资产负债表
    """
    df: pd.DataFrame = ak.stock_zcfz_em(date=date)
    return df


# 57. 资产负债表-北交所
def get_stock_balance_sheet_bj_em(date: str) -> pd.DataFrame:
    """
    获取资产负债表-北交所
    date:date="20200331"; choice of {"XXXX0331", "XXXX0630", "XXXX0930", "XXXX1231"}; 从 20100331 开始
    :return: 资产负债表-北交所
    """
    df: pd.DataFrame = ak.stock_zcfz_bj_em(date=date)
    return df


# 58. 利润表
def get_stock_income_statement_em(date: str) -> pd.DataFrame:
    """
    获取利润表
    date:date="20200331"; choice of {"XXXX0331", "XXXX0630", "XXXX0930", "XXXX1231"}; 从 20100331 开始
    :return: 利润表
    """
    df: pd.DataFrame = ak.stock_lrb_em(date=date)
    return df


# 59. 现金流量表
def get_stock_cash_flow_em(date: str) -> pd.DataFrame:
    """
    获取现金流量表
    date:date="20200331"; choice of {"XXXX0331", "XXXX0630", "XXXX0930", "XXXX1231"}; 从 20100331 开始
    :return: 现金流量表
    """
    df: pd.DataFrame = ak.stock_xjll_em(date=date)
    return df


# 60. 股东增减持
def get_stock_shareholder_em(symbol: str = "全部") -> pd.DataFrame:
    """
    获取股东增减持
    symbol="全部"; choice of {"全部", "股东增持", "股东减持"}
    :return: 股东增减持
    """
    df: pd.DataFrame = ak.stock_ggcg_em(symbol=symbol)
    return df


# 61. 分红配送
def get_stock_dividend_em(date: str) -> pd.DataFrame:
    """
    获取分红配送
    date="20231231"; choice of {"XXXX0630", "XXXX1231"}; 从 19901231 开始
    :return: 分红配送
    """
    df: pd.DataFrame = ak.stock_fhps_em(date=date)
    return df


# 62. 分红配送详情
def get_stock_dividend_detail_em(symbol: str) -> pd.DataFrame:
    """
    获取分红配送详情
    symbol="600000"; 股票代码
    :return: 分红配送详情
    """
    df: pd.DataFrame = ak.stock_fhps_detail_em(symbol=symbol)
    return df


# 63. 个股资金流
def get_stock_fund_flow_em(symbol: str = "即时") -> pd.DataFrame:
    """
    获取个股资金流
    symbol="即时"; choice of {“即时”, "3日排行", "5日排行", "10日排行", "20日排行"}
    :return: 个股资金流
    """
    df: pd.DataFrame = ak.stock_fund_flow_individual(symbol=symbol)
    return df


# 64. 概念资金流
def get_stock_concept_fund_flow_em(symbol: str = "即时") -> pd.DataFrame:
    """
    获取概念资金流
    symbol="即时"; choice of {“即时”, "3日排行", "5日排行", "10日排行", "20日排行"}
    :return: 概念资金流
    """
    df: pd.DataFrame = ak.stock_fund_flow_concept(symbol=symbol)
    return df


# 65. 行业资金流
def get_stock_industry_fund_flow_em(symbol: str = "即时") -> pd.DataFrame:
    """
    获取行业资金流
    symbol="即时"; choice of {“即时”, "3日排行", "5日排行", "10日排行", "20日排行"}
    :return: 行业资金流
    """
    df: pd.DataFrame = ak.stock_fund_flow_industry(symbol=symbol)
    return df


# 66. 大单追踪
def get_stock_big_order_em() -> pd.DataFrame:
    """
    获取大单追踪
    :return: 大单追踪
    """
    df: pd.DataFrame = ak.stock_fund_flow_big_deal()
    return df


# 67. 个股资金流
def get_stock_fund_flow_detail_em(symbol: str, market: str) -> pd.DataFrame:
    """
    获取个股资金流
    symbol="600000"; 股票代码
    ...
    """
    df: pd.DataFrame = ak.stock_individual_fund_flow(stock=symbol, market=market)
    return df


# 68. 个股资金流排名
def get_stock_fund_flow_rank_em(indicator: str) -> pd.DataFrame:
    """
    获取个股资金流排名
    indicator="今日"; choice {"今日", "3日", "5日", "10日"}
    :return: 单次获取指定市场和股票的近 100 个交易日的资金流数据
    """
    df: pd.DataFrame = ak.stock_individual_fund_flow_rank(indicator=indicator)
    return df


# 69. 大盘资金流
def get_stock_market_fund_flow_em() -> pd.DataFrame:
    """
    获取大盘资金流
    :return: 大盘资金流
    """
    df: pd.DataFrame = ak.stock_market_fund_flow()
    return df


# 70. 板块资金流
def get_sector_fund_flow_rank(indicator: str, sector_type: str) -> pd.DataFrame:
    """
    获取板块资金流排名
    indicator="今日"; choice {"今日", "3日", "5日", "10日"}
    ...
    :return: 板块资金流排名
    """
    df: pd.DataFrame = ak.stock_sector_fund_flow_rank(indicator=indicator, sector_type=sector_type)
    return df


# 71. 主力净流入排名
def get_stock_main_fund_flow_rank_em(symbol: str) -> pd.DataFrame:
    """
    获取主力净流入排名
    symbol="全部股票"；choice of {"全部股票", "沪深A股", "沪市A股", "科创板", "深市A股", "创业板", "沪市B股", "深市B股"}
    :return: 主力净流入排名
    """
    df: pd.DataFrame = ak.stock_main_fund_flow(symbol=symbol)
    return df


# 72. 筹码分布
def get_stock_capital_distribution_em(symbol: str, adjust: str) -> pd.DataFrame:
    """
    获取筹码分布
    symbol="600000"; 股票代码
    ...
    :return: 筹码分布
    """
    df: pd.DataFrame = ak.stock_cyq_em(symbol=symbol, adjust=adjust)
    return df


# 73. A股指定日期公告
def get_stock_announcement_em(date: str, symbol: str) -> pd.DataFrame:
    """
    获取A股指定日期公告
    date="20250729"; 日期
    ...
    :return: A股指定日期公告
    """
    df: pd.DataFrame = ak.stock_notice_report(symbol=symbol, date=date)
    return df


# 74. 港股财务指标
def get_stock_hk_finance_indicator_em(symbol: str, indicator: str) -> pd.DataFrame:
    """
    获取港股财务指标
    symbol="00700"; 股票代码
    ...
    :return: 港股财务指标
    """
    df: pd.DataFrame = ak.stock_financial_hk_analysis_indicator_em(
...
# 75. 美股财务指标
def get_stock_us_finance_indicator_em(symbol: str, indicator: str) -> pd.DataFrame:
    """
    获取美股财务指标
    symbol="AAPL"; 股票代码
    indicator="年度"; choice of {"年度", "报告期"}
    :return: 美股财务指标
    """
    df: pd.DataFrame = ak.stock_financial_us_analysis_indicator_em(
        symbol=symbol, indicator=indicator
    )
    return df
...
# 76. A股历史分红
def get_stock_dividend_history_em() -> pd.DataFrame:
    """
    获取A股历史分红
    symbol="600000"; 股票代码
    :return: A股历史分红
    """
    df: pd.DataFrame = ak.stock_history_dividend()
    return df


# 77. 股东数量
def get_stock_shareholder_num_em(symbol: str) -> pd.DataFrame:
    """
    获取股东数量
    symbol=symbol="20230930"; choice of {"最新", 每个季度末}, 其中 每个季度末需要写成 20230930 格式
    :return: 股东数量
    """
    df: pd.DataFrame = ak.stock_zh_a_gdhs(symbol=symbol)
    return df


# 78. 股票列表-A股
def get_stock_list_em(symbol: str, symbol_type: str | None = None) -> pd.DataFrame:
    """
    获取股票列表
...
# 79. 美股和港股的目标价格
def get_stock_target_price_em(symbol: str) -> pd.DataFrame:
    """
    获取美股和港股的目标价格:https://www.ushknews.com/report.html
    symbol="us"; choice of {"us", "hk"}
    :return: 美股和港股的目标价格
    """
    df: pd.DataFrame = ak.stock_price_js(symbol=symbol)
    return df
...
# 81. A股股息率
def get_stock_dividend_rate_lg(symbol: str) -> pd.DataFrame:
    """
    获取A股股息率
    symbol="上证A股"; choice of {"上证A股", "深证A股", "创业板", "科创板"}
    :return: A股股息率
    """
    df: pd.DataFrame = ak.stock_a_gxl_lg(symbol=symbol)
    return df


# 82. 恒生指数股息率
def get_stock_hk_dividend_rate_lg() -> pd.DataFrame:
    """
    :return:
    """
    df: pd.DataFrame = ak.stock_hk_gxl_lg()
    return df


# 83. 大盘拥挤度
def get_stock_market_crowding_lg() -> pd.DataFrame:
    """
    获取大盘拥挤度
    :return: 大盘拥挤度
    """
    df: pd.DataFrame = ak.stock_a_congestion_lg()
    return df


# 84. 股债利差
def get_stock_bond_spread_lg() -> pd.DataFrame:
    """
    获取股债利差
    :return: 股债利差
    """
    df: pd.DataFrame = ak.stock_ebs_lg()
    return df


# 85. 巴菲特指标
def get_stock_baft_indicator_lg() -> pd.DataFrame:
    """
    获取巴菲特指标
    :return: 巴菲特指标
    日期	object	交易日
    ...
    """
    df: pd.DataFrame = ak.stock_buffett_index_lg()
    return df


# 86. A股等权重与中位数市盈率
def get_stock_weighted_median_pe_lg() -> pd.DataFrame:
    """
    获取A股等权重与中位数市盈率
    :return: A股等权重与中位数市盈率
    date	object	日期
    ...
    """
    df: pd.DataFrame = ak.stock_a_ttm_lyr()
    return df


# 87. A股等权重与中位数市净率
def get_stock_weighted_median_pb_lg() -> pd.DataFrame:
    """
    获取A股等权重与中位数市净率
    :return: A股等权重与中位数市净率
    date	object	日期
    ...
    """
    df: pd.DataFrame = ak.stock_a_all_pb()
    return df


# 88. A股主板市盈率
def get_stock_mainboard_pe_lg(symbol: str) -> pd.DataFrame:
    """
    获取A股主板市盈率
    symbol="上证"; choice of {"上证", "深证", "创业板", "科创版"}
    :return: A股主板市盈率
    """
    df: pd.DataFrame = ak.stock_market_pe_lg(symbol=symbol)
    return df


# 89. A股指数市盈率
def get_stock_index_pe_lg(symbol: str) -> pd.DataFrame:
    """
    获取A股指数市盈率
    symbol="上证50"; choice of {"上证50", "沪深300", "上证380", "创业板50", "中证500", "上证180", "深证红利", "深证100",
        "中证1000", "上证红利", "中证100", "中证800"}
    :return: A股指数市盈率
    """
    df: pd.DataFrame = ak.stock_index_pe_lg(symbol=symbol)
    return df


# 90. 主板市净率
def get_stock_mainboard_pb_lg(symbol: str) -> pd.DataFrame:
    """
    获取主板市净率
    symbol="上证"; choice of {"上证", "深证", "创业板", "科创版"}
    :return: 主板市净率
    """
    df: pd.DataFrame = ak.stock_market_pb_lg(symbol=symbol)
    return df


# 91. 指数市净率
def get_stock_index_pb_lg(symbol: str) -> pd.DataFrame:
    """
    获取指数市净率
    symbol="上证50";
    choice of {"上证50", "沪深300", "上证380", "创业板50", "中证500", "上证180", "深证红利", "深证100",
        "中证1000", "上证红利", "中证100", "中证800"}
    :return: 指数市净率
    """
    df: pd.DataFrame = ak.stock_index_pb_lg(symbol=symbol)
    return df


# 92. A股估值指标
def get_stock_valuation_baidu(symbol: str, indicator: str, period: str) -> pd.DataFrame:
    """
    ...
    :return:
    """
    df: pd.DataFrame = ak.stock_zh_valuation_baidu(
...
# 93. 个股估值指标
def get_stock_valuation_em(symbol: str) -> pd.DataFrame:
    """
    symbol    str    symbol="002044"; A 股代码
    :return:
    """
    df: pd.DataFrame = ak.stock_value_em(symbol=symbol)
    return df


# 94. 百度情绪指标，股民投票
def get_stock_baidu_trend(symbol: str, indicator: str) -> pd.DataFrame:
    """
     symbol="002044"; A 股代码或指数代码
     indicator= "指数"; choice of {"指数", "股票"}
    :return:
    """
    df: pd.DataFrame = ak.stock_zh_vote_baidu(symbol=symbol, indicator=indicator)
    return df


# 95. 港股个股指标
def get_stock_hk_indicator_baidu(symbol: str, indicator: str, period: str) -> pd.DataFrame:
    """
    获取港股个股指标
    symbol="hk01093"
    ...
    :return: 港股个股指标
    """
    df: pd.DataFrame = ak.stock_hk_valuation_baidu(
        symbol=symbol, indicator=indicator, period=period
    )
    return df
...
# 96. A股创新高和新低的股票数量
def get_stock_new_high_low(symbol: str) -> pd.DataFrame:
    """
    获取A股创新高和新低的股票数量
    :param symbol:="all"; {"all": "全部A股", "sz50": "上证50", "hs300": "沪深300", "zz500": "中证500"}
    :return:
    """
    df: pd.DataFrame = ak.stock_a_high_low_statistics(symbol=symbol)
    return df


# 97. A股破净值统计
def get_stock_below_net_statistics(symbol: str) -> pd.DataFrame:
    """
    获取A股破净值统计
    ...
    :return:
    """
    df: pd.DataFrame = ak.stock_a_below_net_asset_statistics(symbol=symbol)
    return df


# 98. 基金持股明细
def get_fund_hold_detail(symbol: str, date: str) -> pd.DataFrame:
    """
    获取基金持股明细
    symbol="000001"; 基金代码
    ...
    :return:
    """
    df: pd.DataFrame = ak.stock_report_fund_hold_detail(symbol=symbol, date=date)
    return df


# 99. 融资融券名单
def get_stock_margin_ratio(date: str) -> pd.DataFrame:
    """
    获取融资融券名单
    date="20200630" 指定交易日
    :return:
    """
    df: pd.DataFrame = ak.stock_margin_ratio_pa(date=date)
    return df


# 100. 两融账户信息
def get_stock_margin_account_info() -> pd.DataFrame:
    """
    获取两融账户信息
    :return:
    """
    df: pd.DataFrame = ak.stock_margin_account_info()
    return df


# 101. 上交所两融汇总
def get_stock_margin_summary_sh(start_date: str, end_date: str) -> pd.DataFrame:
    """
    获取上交所两融汇总
    ...
    :return:
    """
    df: pd.DataFrame = ak.stock_margin_sse(start_date=start_date, end_date=end_date)
    return df


# 102. 上交所两融明细
def get_stock_margin_detail_sh(date: str) -> pd.DataFrame:
    """
    获取上交所两融明细
    date="20200630"
    :return:
    """
    df: pd.DataFrame = ak.stock_margin_detail_sse(date=date)
    return df


# 103. 深交所两融汇总
def get_stock_margin_summary_sz(date: str) -> pd.DataFrame:
    """
    获取深交所两融汇总
    date="20200630";
    :return:
    """
    df: pd.DataFrame = ak.stock_margin_szse(date=date)
    return df


# 104. 深交所两融明细
def get_stock_margin_detail_sz(date: str) -> pd.DataFrame:
    """
    获取深交所两融明细
    date="20200630";
    :return:
    """
    df: pd.DataFrame = ak.stock_margin_detail_szse(date=date)
    return df


# 105。 同花顺行业列表
def get_stock_industry_list_ths() -> pd.DataFrame:
    """
    获取同花顺行业列表
    :return:
    """
...
# 106. 同花顺指数数据
def get_stock_index_ths(symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
    """
    获取同花顺指数数据
    ...
    :return:
    """
    df: pd.DataFrame = ak.stock_board_industry_index_ths(
...
# 107. 雪球讨论热度最高个股
def get_stock_hot_tweet_xq(symbol: str) -> pd.DataFrame:
    """
    获取雪球讨论热度最高个股
    symbol="最热门"; choice of {"本周新增", "最热门"}
    :return:
    """
    df: pd.DataFrame = ak.stock_hot_tweet_xq(symbol=symbol)
    return df


# 108. 雪球交易排行榜
def get_stock_trade_rank_xq(symbol: str) -> pd.DataFrame:
    """
    获取雪球交易排行榜
    symbol="最热门"; choice of {"本周新增", "最热门"}
    :return:
    ...
    df: pd.DataFrame = ak.stock_hot_deal_xq(symbol=symbol)
    return df


# 109. 股票热度-东财
def get_stock_hot_rank_em() -> pd.DataFrame:
    """
    获取股票热度-东财
    :return:
    """
    df: pd.DataFrame = ak.stock_hot_rank_em()
    return df


# 110. 东财个股飙升榜
def get_stock_rising_rank_em() -> pd.DataFrame:
    """
    获取东财个股飙升榜
    :return:
    """
...
# 111. 港股人气榜-东财
def get_stock_hk_hot_rank_em() -> pd.DataFrame:
    """
    获取港股人气榜-东财
    :return:
    """
...
# 112. 相关股票
def get_stock_related_em(symbol: str) -> pd.DataFrame:
    """
    获取相关股票
    symbol="SZ000665";
    :return:
    """
    df: pd.DataFrame = ak.stock_hot_rank_relate_em(symbol=symbol)
    return df


# 113. 昨天涨停股池
def get_stock_yesterday_limit_em(date: str) -> pd.DataFrame:
    """
    获取昨天涨停股池
    date="20200630";指定日期的前一天
    :return:
    """
    df: pd.DataFrame = ak.stock_zt_pool_previous_em(date=date)
    return df


# 114. 强势股
def get_stock_strong_em(date: str) -> pd.DataFrame:
    """

    :param date:
    :return:
    """
    df: pd.DataFrame = ak.stock_zt_pool_strong_em(date=date)
    return df


# 115. 跌停股池
def get_stock_down_limit_em(date: str) -> pd.DataFrame:
    """
    date: '20241011'
    :return:
    """
    df: pd.DataFrame = ak.stock_zt_pool_dtgc_em(date=date)
    return df
...
def get_market_activity() -> pd.DataFrame:
    ...
    :return:
    """
    df: pd.DataFrame = ak.stock_market_activity_legu()
    return df


# 117. 创造新高的个股
def get_stock_new_high(symbol: str) -> pd.DataFrame:
    """
    symbol="创月新高"; choice of {"创月新高", "半年新高", "一年新高", "历史新高"}
    :return:
    """
    df: pd.DataFrame = ak.stock_rank_cxg_ths(symbol=symbol)
    return df


# 118. 创造新低的个股
def get_stock_new_low(symbol: str) -> pd.DataFrame:
    """
    symbol="创月新低"; choice of {"创月新低", "半年新低", "一年新低", "历史新低"}
    :return:
    """
    df: pd.DataFrame = ak.stock_rank_cxd_ths(symbol=symbol)
    return df


# 119. 连续上涨
def get_stock_continue_up() -> pd.DataFrame:
    """
    :return:
    """
    df: pd.DataFrame = ak.stock_rank_lxsz_ths()
    return df


# 120. 连续下跌
def get_stock_continue_down() -> pd.DataFrame:
    """
    :return:
    """
    df: pd.DataFrame = ak.stock_rank_lxxd_ths()
    return df


# 121. 持续放量
def get_stock_continue_up_volume() -> pd.DataFrame:
    """
    :return:
    """
    df: pd.DataFrame = ak.stock_rank_cxfl_ths()
    return df


# 122. 持续缩量
def get_stock_continue_down_volume() -> pd.DataFrame:
    """
    :return:
    """
    df: pd.DataFrame = ak.stock_rank_cxsl_ths()
    return df


# 123. 向上突破
def get_stock_break_up(symbol: str) -> pd.DataFrame:
    """
    symbol="500日均线"; choice of {"5日均线", "10日均线", "20日均线", "30日均线", "60日均线", "90日均线", "250日均线", "500日均线"}
    :return:
    """
    df: pd.DataFrame = ak.stock_rank_xstp_ths(symbol=symbol)
    return df


# 124. 向下突破
def get_stock_break_down(symbol: str) -> pd.DataFrame:
    """
    symbol="500日均线"; choice of {"5日均线", "10日均线", "20日均线", "30日均线", "60日均线", "90日均线", "250日均线", "500日均线"}
    :return:
    """
    df: pd.DataFrame = ak.stock_rank_xxtp_ths(symbol=symbol)
    return df


# 125. 量价齐升
def get_stock_up_pv() -> pd.DataFrame:
    """

    :return:
    """
    df: pd.DataFrame = ak.stock_rank_ljqs_ths()
    return df


# 126. 量价齐跌
def get_stock_down_pv() -> pd.DataFrame:
    """
    :return:
    """
    df: pd.DataFrame = ak.stock_rank_ljqd_ths()
    return df
    ...
    # print(get_stock_minute_em(symbol='000001', start_date='2025-07-01 09:32:00', end_date='2225-07-29 09:32:00',
    #   period='1', adjust='qfq'))
    ...
    # print(get_stock_us_history(symbol='106.TTE', period='daily', start_date='20150101', end_date='20250729',
    #   adjust='hfq'))
    # print(get_stock_hk_realtime_quotes())
    # print(get_stock_hk_main_realtime_quotes())
    # print(get_stock_hk_minute_em(symbol='00700', period='1', start_date='2025-07-01 09:32:00',
    #   end_date='2025-07-29 09:32:00', adjust=''))
    # print(get_stock_hk_history_em(symbol='00700', period='daily', start_date='20150101', end_date='20250729',
    #   adjust=''))
