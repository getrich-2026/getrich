...
from getrich.apps.web.deps import get_current_user, get_db, request_id, require_user
from getrich.apps.web.response import success
from getrich.apps.web.schemas.subscription import SubscribeIn, UnsubscribeIn
from getrich.apps.web.services import subscription as svc
...
async def subscribe(
    ...
    data = await svc.subscribe(
        db, user_id=user_id, strategy_code=strategy_code, body=body,
    )
...
async def unsubscribe(
    ...
    data = await svc.unsubscribe(
        db, user_id=user_id, strategy_code=strategy_code,
