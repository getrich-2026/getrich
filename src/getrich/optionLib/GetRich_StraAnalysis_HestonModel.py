...
class HestonModel:
    ...
    def simulate(self, days: int, n_paths: int) -> np.ndarray:
        ...
        for t in range(1, days + 1):
            ...
            dW2 = (self.params['rho'] * epsilon1 + np.sqrt(1 - self.params['rho'] ** 2) * epsilon2) * np.sqrt(dt)
            # 更新波动率路径（欧拉离散化）
            sqrt_v_prev = np.sqrt(np.maximum(v_prev, 0.01))  # 防止负波动率,设置不小于1%
            v_new = v_prev + self.params["kappa"] * (self.params["theta"] - v_prev) * dt + self.params[
                "sigma_v"] * sqrt_v_prev * dW2
            v_new = np.maximum(v_new, 0.01)  # 确保波动率始终为正,不小于1%
            v_paths[:, t] = v_new  # 存储更新后的波动率
            # 更新资产价格路径（对数欧拉方法）
            drift = (self.params["r"] - self.params["q"] - 0.5 * np.maximum(v_prev, 0.01)) * dt  # 漂移项
