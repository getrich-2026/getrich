...
SYMBOL_INFO_CONFIG = {
    'stock': {
        'folder': '股票',
        'columns': ['order_book_id', 'symbol', 'exchange', 'listed_date', 'de_listed_date'],
    ...
    'future': {
        'folder': '期货',
        'columns': ['order_book_id', 'symbol', 'exchange', 'listed_date', 'maturity_date', 
                    'contract_multiplier', 'underlying_order_book_id', 'underlying_symbol'],
        'has_underlying': True,
        'has_option_fields': False,
...
def _process_ricequant_symbol_info(tempdata: pd.DataFrame, asset: str, date: str) -> pd.DataFrame:
    """
    处理 RiceQuant 证券信息数据
    ...
    if config['has_option_fields']:
        ...
        if 'optiontype' in tempdata.columns:
            tempdata['optiontype'] = tempdata['optiontype'].map({'C': 'call', 'P': 'put', 'c': 'call', 'p': 'put'})
    else:
    ...
    # 确保列顺序一致
    output_cols = ['symbol', 'dt', 'exchange', 'name', 'type', 'und_code', 
                   'und_name', 'optiontype', 'strike', 'multiplier', 
                   'listed_date', 'delisted_date', 'source']
    # 只保留存在的列
...
def _process_ricequant_daily_bar(tempdata: pd.DataFrame, asset: str, has_oi_settle: bool, symbol_info: pd.DataFrame = None) -> pd.DataFrame:
    """
    处理 RiceQuant 日线数据
    ...
    """
    # 基础列
    base_columns = ['order_book_id', 'date', 'open', 'high', 'low', 'close', 'volume', 'total_turnover']
    if has_oi_settle:
    ...
    if 'symbol' in tempdata.columns:
        ...
        if asset in ['future', 'option'] and symbol_info is not None and not symbol_info.empty:
            # 创建纯代码 -> 完整代码的映射
            symbol_mapping = {}
    ...
    # 确保列顺序
    output_cols = ['symbol', 'dt', 'open', 'high', 'low', 'close', 
                   'volume', 'amount', 'open_interest', 'settle', 'type', 'source']
    output_cols = [c for c in output_cols if c in tempdata.columns]
    tempdata = tempdata[output_cols]
...
def _process_ricequant_minute_bar(tempdata: pd.DataFrame, asset: str, has_oi_settle: bool, symbol_info: pd.DataFrame = None) -> pd.DataFrame:
    """
    处理 RiceQuant 分钟数据
    ...
    """
    # 基础列
    base_columns = ['order_book_id', 'datetime', 'trading_date', 'open', 'high', 'low', 'close', 'volume', 'total_turnover']
    if has_oi_settle:
    ...
    # 确保列顺序
    output_cols = ['symbol', 'dt', 'ts', 'open', 'high', 'low', 'close', 
                   'volume', 'amount', 'open_interest', 'settle', 'type', 'source']
    output_cols = [c for c in output_cols if c in tempdata.columns]
    tempdata = tempdata[output_cols]
...
def _refresh_ricequant_daily_data(date: str):
    ...
    for asset in ['future', 'option']:
        ...
        symbol_info_file = miller_data_path / "证券信息" / DAILY_BAR_CONFIG[asset]['folder'] / f"{date}.parquet"
        if symbol_info_file.exists():
            ...
            raw_symbol_info_file = api_data_path / "ricequant_data" / "证券信息" / DAILY_BAR_CONFIG[asset]['folder'] / f"{date}.parquet"
            if raw_symbol_info_file.exists():
                try:
                    raw_symbol_info = pd.read_parquet(raw_symbol_info_file)
                    # 处理原始证券信息数据，补齐 symbol 后缀
                    symbol_info_cache[asset] = _process_ricequant_symbol_info(raw_symbol_info, asset, date)
                except Exception as e:
    ...
    for asset, config in DAILY_BAR_CONFIG.items():
        ...
        try:
            ...
            symbol_info = symbol_info_cache.get(asset) if asset in ['future', 'option'] else None
            
            # 处理数据
            tempdata = _process_ricequant_daily_bar(tempdata, asset, config['has_oi_settle'], symbol_info)
...
def _refresh_ricequant_minute_data(date: str):
    ...
    for asset in ['future', 'option']:
        # 首先尝试从已处理的 miller_data 读取
        symbol_info_file = miller_data_path / "证券信息" / MINUTE_BAR_CONFIG[asset]['folder'] / f"{date}.parquet"
        if symbol_info_file.exists():
            try:
                symbol_info_cache[asset] = pd.read_parquet(symbol_info_file)
            except Exception as e:
            ...
            # 尝试从 api_data 读取原始证券信息并处理
            raw_symbol_info_file = api_data_path / "ricequant_data" / "证券信息" / MINUTE_BAR_CONFIG[asset]['folder'] / f"{date}.parquet"
            if raw_symbol_info_file.exists():
                try:
                    raw_symbol_info = pd.read_parquet(raw_symbol_info_file)
                    # 处理原始证券信息数据，补齐 symbol 后缀
    ...
    for asset, config in MINUTE_BAR_CONFIG.items():
        ...
        try:
            ...
            # 处理数据
            tempdata = _process_ricequant_minute_bar(tempdata, asset, config['has_oi_settle'], symbol_info)
...
def _refresh_insight_symbol_info_impl(date: str):
    ...
    for miller_type, (insight_folder, _) in asset_mapping.items():
        ...
        # 统一列顺序
        tempdata = tempdata[['symbol', 'dt', 'exchange', 'name', 'type', 'und_code', 
                             'und_name', 'optiontype', 'strike', 'multiplier', 
                             'listed_date', 'delisted_date', 'source']]
        
        # 保存到 miller_data
        tempdata.to_parquet(output_folder / f"{date}.parquet", index=False, engine='pyarrow')
...
def _refresh_insight_daily_data(date: str):
    ...
    for miller_type, (insight_folder, has_oi_settle) in asset_mapping.items():
        ...
        # 处理 Insight 日线数据
        tempdata = _process_insight_daily_bar(tempdata, date, miller_type, has_oi_settle)
        
        # 统一列顺序
        tempdata = tempdata[['symbol', 'dt', 'open', 'high', 'low', 'close', 
...
def _refresh_insight_minute_data(date: str):
    ...
    for miller_type, (insight_folder, has_oi_settle) in asset_mapping.items():
        ...
        # 处理 Insight 分钟数据
        tempdata = _process_insight_minute_bar(tempdata, date, miller_type, has_oi_settle)
        
        # 统一列顺序
        tempdata = tempdata[['symbol', 'dt', 'ts', 'open', 'high', 'low', 'close', 
                             'volume', 'amount', 'open_interest', 'settle', 
                             'type', 'source']]
...
def _process_insight_daily_bar(tempdata: pd.DataFrame, date: str, miller_type: str, has_oi_settle: bool) -> pd.DataFrame:
    """
    处理 Insight 日线行情数据
    ...
    """
    # 选择需要的列
    base_columns = ['htsc_code', 'time', 'open', 'close', 'high', 'low', 'volume', 'value']
    if has_oi_settle:
...
def _process_insight_minute_bar(tempdata: pd.DataFrame, date: str, miller_type: str, has_oi_settle: bool) -> pd.DataFrame:
    """
    处理 Insight 分钟行情数据
...
def _process_insight_bond(tempdata: pd.DataFrame, date: str) -> pd.DataFrame:
    """处理 Insight 可转债数据"""
    columns_needed = ['htsc_code', 'name', 'exchange', 'exchange_code', 'listing_date', 
                      'convert_code', 'convert_abbr']
    available_cols = [c for c in columns_needed if c in tempdata.columns]
    tempdata = tempdata[available_cols].copy()
...
def _process_insight_future(tempdata: pd.DataFrame, date: str) -> pd.DataFrame:
    """处理 Insight 期货数据"""
    columns_needed = ['htsc_code', 'name', 'exchange', 'listing_date', 'expire_date',
                      'product_id', 'instrument_name', 'volume_multiple']
    available_cols = [c for c in columns_needed if c in tempdata.columns]
    tempdata = tempdata[available_cols].copy()
    ...
    if 'multiplier' in tempdata.columns:
        tempdata['multiplier'] = pd.to_numeric(tempdata['multiplier'], errors='coerce').fillna(1)
    else:
        tempdata['multiplier'] = 1
    
    if 'und_code' not in tempdata.columns or tempdata['und_code'].isna().all():
        tempdata['und_code'] = tempdata['symbol'].str.replace(r'\.\w+$', '', regex=True).str.replace(r'\d+', '', regex=True)
...
def _process_insight_option(tempdata: pd.DataFrame, date: str) -> pd.DataFrame:
    """处理 Insight 期权数据"""
    columns_needed = ['htsc_code', 'name', 'exchange', 'listing_date', 'option_end_date',
                      'option_underlying_security_id', 'option_underlying_symbol',
                      'option_call_or_put', 'option_exercise_price', 'option_contract_multiplier_unit']
    available_cols = [c for c in columns_needed if c in tempdata.columns]
    tempdata = tempdata[available_cols].copy()
    ...
    if 'multiplier' in tempdata.columns:
        tempdata['multiplier'] = pd.to_numeric(tempdata['multiplier'], errors='coerce').fillna(10000)
    else:
