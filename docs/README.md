...
| Agent／维护者解释取舍 | [DECISIONS.md](../.agents/brain/DECISIONS.md) | 按主题查仍有价值的理由，编号稳定；不要求全文加载 |
...
7. **移动必须修引用**：同步 Markdown 链接、指令入口和注释中的路径；已记账 SQL 不为改文档路径而变更 checksum。删决策要修编号引用，不重排或复用旧编号。

本次整理于 2026-09-08 按用户授权删除过期决定；后续日常维护仍以追加／明确取代为主，删除须获授权。
