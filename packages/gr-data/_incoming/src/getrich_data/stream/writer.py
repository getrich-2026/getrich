...
from getrich_data.stream.base import TickEvent

log = get_logger("stream.writer")

_COLUMNS = (
    "instrument_id", "dt", "trading_day", "last", "volume", "amount",
    "bid1", "ask1", "bid_vol1", "ask_vol1", "open_interest", "source",
)
...
class PgTickWriter:
    ...
    def write(self, events: Sequence[TickEvent]) -> int:
        ...
        rows = [
            (
                e.instrument_id, e.dt, e.trading_day, e.last, e.volume, e.amount,
                e.bid1, e.ask1, e.bid_vol1, e.ask_vol1, e.open_interest, e.source,
            )
            for e in events
        ]
