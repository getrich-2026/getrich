...
接线点（需改既有文件）：
- `src/gr_data/raw/__init__.py`：`PROVIDERS` 加 `"tushare"`；`_registry`/`_build_client` 加分支。
- `src/gr_data/ingest/__init__.py`：`PROVIDERS` 加 `"tushare"`；`_registry_groups` 加分支。
- `src/gr_data/cli.py`：`raw`/`ingest` 子命令 `choices` 加 `"tushare"`（`raw.py:115`、`ingest.py:121`）。stream 不涉及（Tushare 本期不做实时）。
