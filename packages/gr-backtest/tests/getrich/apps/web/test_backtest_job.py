...
from getrich.apps.web.services.backtest_job import (
    _body_fingerprint,
    _dt,
    _dt_nullable,
    _job_detail,
...
class TestBodyFingerprint:
    ...
    def test_pydantic_with_field_reorder_same_fingerprint(self) -> None:
        ...
        a = _SampleRequest(strategy_id="s-1", symbols=["A"], initial_cash=Decimal("1"), start_at=datetime(2024, 1, 1))
        b = _SampleRequest(start_at=datetime(2024, 1, 1), initial_cash=Decimal("1"), symbols=["A"], strategy_id="s-1")
        assert _body_fingerprint(a) == _body_fingerprint(b)

    def test_different_strategy_different_fingerprint(self) -> None:
        body_a = _SampleRequest(strategy_id="s-1", symbols=["A"], initial_cash=Decimal("1"), start_at=datetime(2024, 1, 1))
        body_b = _SampleRequest(strategy_id="s-2", symbols=["A"], initial_cash=Decimal("1"), start_at=datetime(2024, 1, 1))
        assert _body_fingerprint(body_a) != _body_fingerprint(body_b)

    def test_different_initial_cash_different_fingerprint(self) -> None:
        body_a = _SampleRequest(strategy_id="s-1", symbols=["A"], initial_cash=Decimal("100"), start_at=datetime(2024, 1, 1))
        body_b = _SampleRequest(strategy_id="s-1", symbols=["A"], initial_cash=Decimal("200"), start_at=datetime(2024, 1, 1))
        assert _body_fingerprint(body_a) != _body_fingerprint(body_b)
    ...
    def test_decimal_value_preserved_canonically(self) -> None:
        ...
        body_a = _SampleRequest(strategy_id="s-1", symbols=["A"], initial_cash=Decimal("100.5"), start_at=datetime(2024, 1, 1))
        body_b = _SampleRequest(strategy_id="s-1", symbols=["A"], initial_cash=Decimal("100.5"), start_at=datetime(2024, 1, 1))
    ...
    def test_server_stamped_keys_are_stripped(self) -> None:
        ...
        fingerprint equal."""
        body_a = _SampleRequest(strategy_id="s-1", symbols=["A"], initial_cash=Decimal("1"), start_at=datetime(2024, 1, 1))
        body_b = _SampleRequest(strategy_id="s-1", symbols=["A"], initial_cash=Decimal("1"), start_at=datetime(2024, 1, 1))
...
class TestCrossFormatterConsistency:
    def test_summary_contains_all_job_columns(self) -> None:
        ...
        expected_keys = {
            "job_id", "job_type", "ref_id", "status", "progress",
