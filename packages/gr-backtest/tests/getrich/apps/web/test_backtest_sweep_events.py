"""Tests for the GET /backtest-sweeps/{sweep_id}/events SSE endpoint.

The endpoint streams lifecycle events for one sweep, keyed on the
*sweep* row (not the parent job):
...
The wire format mirrors :func:`backtest_job.stream_job_events` but the
``data:`` payload is the :func:`_sweep_detail` (sweep row + parent job
merged in) so the page can ``setQueryData`` the whole row in one shot.

Most tests in this file exercise the async generator
``sweep_svc.stream_sweep_events`` directly with a fake ``Request``.
...
from getrich.apps.web.routers.backtest_sweeps import router
from getrich.apps.web.services import backtest_sweep as sweep_svc
...
def _sweep_row(
    ...
    **overrides: Any,
    ...
    """Build a fake ``backtest_sweeps`` row matching ``_SELECT_SWEEP_SQL``."""
    row: dict[str, Any] = {
        "sweep_id": sweep_id,
        "user_id": user_id,
        "search_type": "grid",
...
def _fast_poll(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    The stream reads the sweep from the store and the parent job from
    ``_JOB_STORE``. Both modules re-import the cadence constants from
    ``sse.py`` as private aliases — patching the originals makes the
    aliases pick up the test values transitively.
    """
...
async def test_terminal_at_connect_emits_snapshot_then_done(
    ...
    """Sweep whose parent job is already completed at connect time:
    snapshot + done + generator returns. No poll-loop run needed."""
    sweep = _sweep_row(status="completed", completed_trials=4)
    job = _job_row(status="completed", progress=100)

    async def fake_get_sweep(sweep_id, user_id=None, conn=None):
        return sweep

    async def fake_get_job_by_ref_id(ref_id, user_id=None, conn=None):
        return job

    monkeypatch.setattr(sweep_svc._STORE, "get_sweep", fake_get_sweep)
    monkeypatch.setattr(sweep_svc._JOB_STORE, "get_job_by_ref_id", fake_get_job_by_ref_id)

    request = _FakeRequest()
    gen = sweep_svc.stream_sweep_events(sweep_id="sweep-1", user_id="user-1", request=request)
...
async def test_active_to_terminal_emits_update_chain(
    ...
    """Active sweep emits snapshot + N updates + done as the runner writes.

    The terminal frame is keyed on the *parent job*'s status, not the
    sweep row's status (the runner only writes the sweep row AFTER
    finishing — using the sweep's own status would race the writer).
    """
    # Distinct ``updated_at`` per step — the de-dup etag includes the
    # row's ``updated_at`` so identical timestamps would suppress the
    # update frames. In production, the runner writes a fresh
    # ``updated_at`` on every progress tick.
    sweeps = [
        _sweep_row(status="running", completed_trials=0, updated_at=datetime(2026, 6, 4, 9, 30, 0)),
        _sweep_row(status="running", completed_trials=1, updated_at=datetime(2026, 6, 4, 9, 30, 1)),
        _sweep_row(status="running", completed_trials=3, updated_at=datetime(2026, 6, 4, 9, 30, 2)),
        _sweep_row(
            status="completed", completed_trials=4, updated_at=datetime(2026, 6, 4, 9, 30, 3)
        ),
    ]
    jobs = [
    ...
    async def fake_get_job_by_ref_id(ref_id, user_id=None, conn=None):
        i = max(0, idx["n"] - 1)  # paired with the sweep read
...
async def test_cancellation_via_parent_job_emits_done(
    ...
    (the sweep row's enum has no ``cancelled`` value)."""
    sweep_running = _sweep_row(status="running", completed_trials=2)
    job_running = _job_row(status="running", progress=50)
    job_cancelled = _job_row(status="cancelled", progress=50)
    pairs = [(sweep_running, job_running), (sweep_running, job_cancelled)]
    idx = {"n": 0}
    ...
    # The sweep's own status remains running — the cancel is a
    # top-level cancellation, not a sweep-lifecycle outcome.
...
async def test_mid_stream_db_error_emits_error_then_close(
    ...
    sweep = _sweep_row(status="running")
    job = _job_row(status="running", progress=10)

    async def fake_get_sweep(sweep_id, user_id=None, conn=None):
        fake_get_sweep.calls = getattr(fake_get_sweep, "calls", 0) + 1
        if fake_get_sweep.calls == 1:
            return sweep
...
async def test_sweep_row_vanished_mid_stream_emits_cancelled_done(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A sweep returning None mid-stream is treated as cancelled + done."""
    sweep_first = _sweep_row(status="running", completed_trials=1)
    ...
    kinds = [e["event"] for e in events]
    assert kinds[-1] == "done"
    # The synthetic cancelled frame mirrors the original sweep's shape
    # so the page can render a coherent terminal row.
    assert events[-1]["data"]["status"] == "cancelled"
    assert events[-1]["data"]["sweep_id"] == "sweep-1"
    assert events[-1]["data"]["completed_trials"] == 1


async def test_initial_sweep_missing_raises_not_found(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Missing sweep at connect time → NotFound (no SSE body opened)."""
    ...
    request = _FakeRequest()
    gen = sweep_svc.stream_sweep_events(sweep_id="missing", user_id="user-1", request=request)
...
def _build_app(*, user_id: str | None = "user-1") -> FastAPI:
    """Build a minimal FastAPI app with the backtest-sweeps router + overrides."""
...
async def test_http_cross_user_returns_404(
    ...
    """A user requesting someone else's sweep gets 404 (not 403)."""

    async def fake_get_sweep(sweep_id, user_id=None, conn=None):
        return None  # store returns None for cross-user probes
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.get("/v1/backtest-sweeps/sweep-1/events")
...
def test_path_registered() -> None:
    ...
    assert "/backtest-sweeps/{sweep_id}/events" in paths
    events_route = next(
        r for r in router.routes if getattr(r, "path", None) == "/backtest-sweeps/{sweep_id}/events"
    )
...
async def test_stream_emits_id_field_on_every_frame(
    ...
    sweep = _sweep_row(status="completed", completed_trials=4, updated_at=base)
    ...
    events, _hit = await _consume_until(
        sweep_svc.stream_sweep_events(sweep_id="sweep-1", user_id="user-1", request=request),
        until_event="done",
        deadline_s=1.0,
    )
...
class _FakeListener:
    ...
    Mirrors the same shape used in the jobs SSE test. The sweep /
    walk-forward generators key on the **parent job's id** (resolved
    via ``get_job_by_ref_id``), not the URL param, so the test's
    ``notify`` calls target the parent id.
    ...
    NOTIFY happens only after the row is visible, and the listener's
    LISTEN was already established at app startup, so a notification
    is never "lost" between subscribe and set().
    """
...
async def test_listener_wakes_sweep_generator_within_50ms(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """When the listener fires ``notify(parent_job_id)``, the sweep
    generator wakes up and re-reads the parent job row within ~50ms.
    """
    fake_listener = _FakeListener()
    monkeypatch.setattr(sweep_svc, "_LISTENER", fake_listener)

    base = datetime(2026, 6, 5, 10, 0, 0)
    sweep = _sweep_row(status="running", completed_trials=0, updated_at=base)
    # Two distinct job rows for the two poll ticks.
    jobs = [
    ...
    # Notify on the parent job_id; the generator wakes up.
    ...
    # Drive to terminal and drain.
    fake_listener.notify("job-1")
    rest, _hit = await _consume_until(gen, until_event="done", deadline_s=1.0)
    assert rest[-1]["event"] == "done"


async def test_sweep_generator_degrades_without_listener(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """When ``_LISTENER`` is None, the sweep generator falls back to
    the 1Hz polling safety net. Frames still arrive.
    """
    monkeypatch.setattr(sweep_svc, "_LISTENER", None)
    ...
    gen = sweep_svc.stream_sweep_events(
        sweep_id="sweep-1", user_id="user-1", request=_FakeRequest()
    )
    events, _hit = await _consume_until(gen, until_event="done", deadline_s=2.0)
