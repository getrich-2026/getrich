...
:func:`getrich_backtest.job_persistence.sync_is_cancelled_status`).
...
1. The worker process opens one long-lived PostgreSQL connection
   ...
   ``getrich_backtest.job_persistence``).
...
:class:`getrich.apps.web.services.job_listener.BacktestJobListener`:
...
# ``getrich_backtest.job_persistence._NOTIFY_JOB_SQL`` so the
