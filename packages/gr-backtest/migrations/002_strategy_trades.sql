...
-- `SET search_path=frontend`; the query in services/strategy.py uses an
-- unprefixed `strategy_trades` name, so the table must live in `frontend`.

CREATE SCHEMA IF NOT EXISTS frontend;

CREATE TABLE IF NOT EXISTS frontend.strategy_trades (
...
COMMENT ON TABLE frontend.strategy_trades IS
...
CREATE INDEX IF NOT EXISTS idx_strategy_trades_strategy
    ON frontend.strategy_trades(strategy_id, executed_at DESC);
CREATE INDEX IF NOT EXISTS idx_strategy_trades_signal
    ON frontend.strategy_trades(signal_id);
