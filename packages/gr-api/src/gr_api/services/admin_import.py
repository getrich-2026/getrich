...
async def _validate_daily_return_rows(
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        if not code:
            row_errors.append(
                _err(idx, "strategy_code", "required", "strategy_code is required", row)
            )
        ...
        benchmark_daily_return = _parse_float_optional(
            row.get("benchmark_daily_return"),
            idx,
...
async def _validate_signal_rows(
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        published_at = _parse_datetime(
            row.get("published_at"), idx, "published_at", row, row_errors
        )
        expire_at = _parse_datetime_optional(
            row.get("expire_at"), idx, "expire_at", row, row_errors
        )
        ...
        confidence = _parse_float_optional(
            row.get("confidence"), idx, "confidence", row, row_errors
        )
        position_pct = _parse_float_optional(
            row.get("position_pct"), idx, "position_pct", row, row_errors
        )
        if confidence is not None and not 0 <= confidence <= 1:
            row_errors.append(
                _err(idx, "confidence", "out_of_range", "confidence must be in [0, 1]", row)
            )
        ...
        decimal_fields = {
            "trigger_price": _parse_decimal_optional(
                row.get("trigger_price"), idx, "trigger_price", row, row_errors
            ),
            "target_price": _parse_decimal_optional(
                row.get("target_price"), idx, "target_price", row, row_errors
            ),
            "stop_loss_price": _parse_decimal_optional(
                row.get("stop_loss_price"), idx, "stop_loss_price", row, row_errors
            ),
        }
        suggested_quantity = _parse_int_optional(
            row.get("suggested_quantity"),
            idx,
            "suggested_quantity",
            row,
            row_errors,
...
async def _load_strategy_map(
    ...
    async with db.cursor() as cur:
        ...
        rows = await cur.fetchall()
    return {row["strategy_code"]: {"id": str(row["id"]), "market": row["market"]} for row in rows}
    ...
    if value not in allowed:
        errors.append(
            _err(
                ...
                column,
                "invalid_enum",
                f"{column} must be one of {sorted(allowed)}",
                raw_row,
            )
        )
...
def _parse_decimal_optional(
    ...
    if not parsed.is_finite():
        errors.append(
            _err(row_number, column, "invalid_decimal", "NaN and Inf are not allowed", raw_row)
        )
        return None
    return parsed
...
def _parse_json(
    ...
    if not isinstance(parsed, dict):
        errors.append(
            _err(row_number, column, "invalid_json", "JSON value must be an object", raw_row)
        )
        return {}
    return parsed
