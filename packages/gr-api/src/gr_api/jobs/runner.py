...
* ``_is_cancelled``: a sync callable ``() -> bool`` that probes the DB row
  ...
  :func:`gr_api.jobs.persistence.sync_is_cancelled_status` so
...
class BacktestJobRunner:
    ...
    def _make_is_cancelled_fn(self, job_id: str) -> Callable[[], bool]:
        ...
        if not self.db_conninfo:
            return lambda: False
        # Lazy import to break a circular import: ``gr_signal``
        # is imported from ``gr_api.jobs.persistence`` via the
        ...
        from gr_api.jobs.persistence import sync_is_cancelled_status
