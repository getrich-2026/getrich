...
class TestPreviewImport:
    ...
    async def test_daily_returns_with_validation_errors(self) -> None:
        ...
        cur = _FakeCursor()
        cur.set_rows([{"id": "strat-uuid-1", "strategy_code": "STRAT_TEST_001", "market": "cn"}])
        cur.push_one({"cnt": 0})
        conn = _FakeConn(cur)
