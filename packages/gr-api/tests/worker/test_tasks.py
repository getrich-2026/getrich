...
from gr_api.worker import tasks as worker_tasks
...
def _stub_dependencies(monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    monkeypatch.setattr(
        worker_tasks, "init_cancel_listener", lambda: calls.setdefault("init_listener", 1)
    )
    monkeypatch.setattr(
        worker_tasks, "close_cancel_listener", lambda: calls.setdefault("close_listener", 1)
    )
    return calls
