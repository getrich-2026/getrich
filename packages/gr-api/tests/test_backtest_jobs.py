...
from gr_api.services import backtest_job
...
def test_cancel_job_returns_refreshed_detail() -> None:
    ...
    assert any(
        "UPDATE backtest.backtest_jobs" in sql and "cancelled" in sql for sql, _ in cursor.executed
    )
...
def test_cancel_job_uses_db_transport(monkeypatch: Any) -> None:
    ...
    from gr_api.jobs import runner as backtest_job_runner
...
def test_create_backtest_job_with_same_idempotency_key_returns_existing(
    ...
    """
    from gr_api.schemas.backtest import BacktestRunRequest
    from gr_api.services.backtest_job import _body_fingerprint
