...
import pytest
from gr_api.services import job_listener
from gr_api.services.job_listener import (
    CHANNEL,
    BacktestJobListener,
)
...
def _patch_connect(monkeypatch: pytest.MonkeyPatch, *conns: _FakeConn) -> list[_FakeConn]:
    ...
    monkeypatch.setattr(
        "gr_api.services.job_listener.psycopg.AsyncConnection.connect",
        factory,
    )
    return list(conns)
...
async def test_pump_reconnects_after_notifies_error(
    ...
    listener = BacktestJobListener()
    with caplog.at_level("WARNING", logger="gr_api.services.job_listener"):
        await listener.start()
