...
import pytest
from gr_api.jobs.persistence import BacktestJobError, PgBacktestJobStore
...
def test_job_persistence_lives_in_gr_api_not_the_engine() -> None:
    """作业队列属于 gr-api，回测引擎不再导出它。

    引擎是最底层的库；反向导出会把 gr-api 拖进纯计算路径，
    单独安装 gr-backtest 会直接 ImportError。
    """
    from gr_api.jobs.persistence import (
    ...
    assert not hasattr(gr_backtest, "PgBacktestJobStore")
    assert not hasattr(gr_backtest, "BacktestJobError")
