...
class OptionPricer:
    ...
    @staticmethod
    def black_scholes(
        S: float, K: float, T: float, sigma: float, r: float, q: float, option_type: str
    ) -> float:
        ...
        # 根据期权类型计算理论价格
        if option_type == "call":
            price = S * np.exp(-q * T) * stats.norm.cdf(d1) - K * np.exp(-r * T) * stats.norm.cdf(
                d2
            )
        else:
            price = K * np.exp(-r * T) * stats.norm.cdf(-d2) - S * np.exp(-q * T) * stats.norm.cdf(
                -d1
            )
    ...
    def calculate_greeks(
        ...
        if american:
            ...
            price_up = OptionPricer.binomial_tree(
                S * (1 + h), K, T, sigma, r, q, option_type, steps
            )
            price_down = OptionPricer.binomial_tree(
                S * (1 - h), K, T, sigma, r, q, option_type, steps
            )
        else:
        ...
        # 计算Vega：波动率变动的影响
        if american:
            price_sigma_up = OptionPricer.binomial_tree(
                S, K, T, sigma + h, r, q, option_type, steps
            )
        else:
