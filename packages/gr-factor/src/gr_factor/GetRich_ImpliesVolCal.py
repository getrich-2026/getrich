...
class Option(ABC):
    ...
    def implied_volatility(
        ...
        if self.moneyness in ["deep_itm", "deep_otm"]:
            warnings.warn(
                f"警告：深度{'实值' if 'itm' in self.moneyness else '虚值'}期权({self.moneyness})"
                f"可能导致隐含波动率计算不稳定",
                stacklevel=2,
            )
        ...
        try:
            ...
            # 处理深度实值/虚值期权的计算失败问题
            warnings.warn(f"无法计算隐含波动率: {e}", stacklevel=2)
            return np.nan

    def _bisection(
        self, market_price: float, tol: float, max_iter: int, max_volatility: float
    ) -> float:
        """
    ...
    def _newton(
        ...
        for _ in range(max_iter):
            ...
            if abs(v) < min_vega:
                # 尝试切换方法或调整初始值
                sigma = min(sigma * 2, max_volatility) if sigma < 0.1 else max(sigma * 0.5, 0.001)
                v = self.vega(sigma)
                if abs(v) < min_vega:
    ...
    def _py_vollib_iv(self, market_price: float) -> float:
        ...
        try:
            ...
            # 处理深度实值/虚值期权的计算失败问题
            warnings.warn(f"py_vollib无法计算隐含波动率: {e}", stacklevel=2)
            return np.nan
...
class EuropeanOption(Option):
    ...
    def __init__(
        self, S: float, K: float, T: float, r: float, option_type: str = "call", q: float = 0.0
    ):
        """
        初始化欧式期权
        参数:
...
class AmericanOption(Option):
    ...
    def price(self, sigma: float, method: str = "tree") -> float:
        ...
        if method == "baw":
            if self.option_type == "call":
                ...
                warnings.warn("BAW模型不适用于美式看跌期权，使用二叉树定价", stacklevel=2)
        ...
        for t in range(self.steps - 1, -1, -1):
            for j in range(t + 1):
                ...
                hold = np.exp(-self.r * dt) * (
                    p * prices[j, t + 1] + (1 - p) * prices[j + 1, t + 1]
                )
    ...
    def _baw_american_call(self, sigma: float) -> float:
        ...
        if self.q == 0:
            # 无分红时美式看涨期权等价于欧式看涨期权
            d1 = (np.log(self.S / self.K) + (self.r + 0.5 * sigma**2) * self.T) / (
                sigma * np.sqrt(self.T)
            )
            d2 = d1 - sigma * np.sqrt(self.T)
        ...
        # 计算d1
        d1_star = (np.log(S_star / self.K) + (self.r - self.q + 0.5 * sigma_sq) * self.T) / (
            sigma * np.sqrt(self.T)
        )
        ...
        if S_star > self.S:
            # 计算欧式期权价格
            d1 = (np.log(self.S / self.K) + (self.r - self.q + 0.5 * sigma_sq) * self.T) / (
                sigma * np.sqrt(self.T)
            )
            d2 = d1 - sigma * np.sqrt(self.T)
            c_bs = self.S * np.exp(-self.q * self.T) * norm.cdf(d1) - self.K * np.exp(
                -self.r * self.T
            ) * norm.cdf(d2)
...
if __name__ == "__main__":
    ...
    euro_call = EuropeanOption(
        S=4083.2, K=4100, T=41 / 365, r=0.03, option_type="call"
    )  # 沪深300股指期权，9月合约，平值期权，看涨期权，wind给的iv是14.5%
    ...
    # =====================
    print("深度实值看涨期权测试")
    deep_itm_call = EuropeanOption(
        S=4083.2, K=3800, T=0.5, r=0.03, option_type="call"
    )  # wind给出的18%左右
    ...
    print("\n尝试使用数值方法计算深度实值期权...")
    iv_bisect = deep_itm_call.implied_volatility(
        market_price, method="bisection", max_volatility=10.0
    )
    ...
    american_put = AmericanOption(
        S=76960, K=77000, T=67 / 365, r=0.03, option_type="call"
    )  # wind给44.56%
