...
class ScenarioAnalyzer:
    ...
    def _calculate_position_pnl(self, S: float, sigma: float, T: float) -> dict:
        ...
        # 计算标的资产的盈亏（考虑方向）
        und_pnl = (
            (S - self.und_positions["price"])
            * self.und_positions["quantity"]
            * self.und_positions["multiplier"]
        )
    ...
    def _scenario_analysis(self, n_scenarios: int, days_later: int) -> pd.DataFrame:
        ...
        # 初始化结果列表
        records = []
        for pc, vc in zip(price_changes, vol_changes, strict=False):
            # 计算模拟的标的资产价格和波动率
            new_S = self.market_params["S0"] * (1 + pc)  # 模拟的标的资产价格
            new_vol = self.market_params["v0"] * (1 + vc)  # 模拟的波动率
            # 计算模拟的剩余到期时间
            new_T = max(
                self.opt_positions[0]["days_to_expiry"] / 365 - days_later / 365, 1 / 365
            )  # 模拟的剩余到期时间
            # 计算情景下的总盈亏和希腊值
            result = self._calculate_position_pnl(S=new_S, sigma=new_vol, T=new_T)
            # 记录结果
            records.append(
                {"price_change": pc, "vol_change": vc, "pnl": result["pnl"], **result["greeks"]}
            )
        # 将结果转换为DataFrame
