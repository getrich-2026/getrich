...
TZ = __import__("gr_backtest", fromlist=["get_shanghai_tz"]).get_shanghai_tz()
...
class TestCliMain:
    def test_missing_strategy_env_var_returns_1(self):
        ...
        with patch("gr_signal.cli._is_trading_time", return_value=True):
            ret = _run(main())
    ...
    def test_outside_market_hours_returns_0(self):
        ...
        with patch("gr_signal.cli._is_trading_time", return_value=False):
            ret = _run(main())
    ...
    def test_happy_path_returns_0_and_emits_ok(self):
        ...
        with (
            patch("gr_signal.cli._is_trading_time", return_value=True),
            patch("gr_signal.cli.pg_pool") as mock_pool,
            patch("gr_signal.cli.get_registry") as mock_get_reg,
            patch("gr_signal.cli.LiveSignalRunner", return_value=mock_runner),
        ):
