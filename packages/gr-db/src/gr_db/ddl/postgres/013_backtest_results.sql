...
COMMENT ON TABLE backtest.backtest_metrics IS
    'Typed and JSONB performance metrics for completed backtest runs.';

CREATE TABLE IF NOT EXISTS backtest.backtest_equity_points (
...
COMMENT ON TABLE backtest.backtest_equity_points IS
    'Chartable equity time series points persisted from BacktestResult.equity_curve.';
COMMENT ON COLUMN backtest.backtest_equity_points.row_json IS
    'Additional equity columns not promoted to typed columns.';

CREATE INDEX IF NOT EXISTS idx_backtest_equity_points_run_dt
    ON backtest.backtest_equity_points(run_id, dt);
CREATE INDEX IF NOT EXISTS idx_backtest_equity_points_run_strategy_dt
    ON backtest.backtest_equity_points(run_id, strategy_name, dt);

CREATE TABLE IF NOT EXISTS backtest.backtest_final_positions (
    run_id          VARCHAR(128)    NOT NULL,
...
COMMENT ON TABLE backtest.backtest_final_positions IS
    'Final account position snapshot for a completed backtest run.';

CREATE TABLE IF NOT EXISTS backtest.backtest_artifacts (
...
COMMENT ON TABLE backtest.backtest_artifacts IS
...
CREATE INDEX IF NOT EXISTS idx_backtest_artifacts_run
    ON backtest.backtest_artifacts(run_id);
CREATE INDEX IF NOT EXISTS idx_backtest_artifacts_run_type
    ON backtest.backtest_artifacts(run_id, artifact_type);
