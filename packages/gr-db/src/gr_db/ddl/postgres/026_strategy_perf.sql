...
CREATE TABLE IF NOT EXISTS app.strategy_performance_snapshot (
...
CREATE TABLE IF NOT EXISTS app.strategy_equity_curve (
    strategy_id         UUID            NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_equity_curve_strategy_date
    ON app.strategy_equity_curve(strategy_id, trade_date DESC);

COMMENT ON TABLE app.strategy_performance_snapshot IS
    'Periodic performance snapshots for strategies — one row per strategy per date.';
COMMENT ON TABLE app.strategy_equity_curve IS
