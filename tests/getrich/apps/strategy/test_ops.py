...
from getrich.apps.strategy.errors import BacktestJobError
from getrich.apps.strategy.ops import (
    BacktestRunOp,
    SweepRunOp,
    WalkForwardRunOp,
    get_op_for_job_type,
...
def test_backtest_run_op_persists_result_and_returns_progress() -> None:
    ...
    with (
        patch("getrich.apps.strategy.ops._build_bar_loader", return_value=_loader()),
        patch(
            "getrich.apps.strategy.ops.PgBacktestResultStore.save_result",
            new=AsyncMockSave(),
        ) as save_mock,
    ):
        result = _run(op(job))
...
def test_sweep_run_op_persists_sweep_and_returns_progress() -> None:
    ...
    with (
        patch("getrich.apps.strategy.ops._build_bar_loader", return_value=_loader()),
        patch(
            "getrich.apps.strategy.ops.PgSweepResultStore.save_sweep_result",
            new=AsyncMockSave(),
        ) as save_mock,
    ):
        result = _run(op(job))
...
def test_walk_forward_run_op_persists_wf_and_returns_progress() -> None:
    ...
    with (
        patch("getrich.apps.strategy.ops._build_bar_loader", return_value=_loader()),
        patch(
            "getrich.apps.strategy.ops.PgWalkForwardResultStore.save_walk_forward_result",
            new=AsyncMockSave(),
        ) as save_mock,
    ):
        result = _run(op(job))
...
def test_sweep_run_op_returns_cancelled_without_persisting() -> None:
    ...
    with (
        ...
        patch(
            "getrich.apps.strategy.ops._build_strategy",
