...
from getrich.apps.worker.cancel_listener import (
    CHANNEL,
...
def _patch_connect(monkeypatch: pytest.MonkeyPatch, *conns: _FakeConn) -> list[_FakeConn]:
    ...
    monkeypatch.setattr(
        "getrich.apps.worker.cancel_listener.psycopg.AsyncConnection.connect",
        factory,
    )
    return list(conns)
...
async def test_reconnect_failure_logs_and_keeps_trying(
    ...
    listener = WorkerCancelListener()
    with caplog.at_level("WARNING", logger="getrich.apps.worker.cancel_listener"):
        await listener.start()
