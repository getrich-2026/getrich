...
from getrich.apps.web.services.backtest_walk_forward import (
    _dt,
    _dt_nullable,
    _empty_wf_row,
    _equity_point,
...
class TestCrossFormatterConsistency:
    def test_summary_contains_all_wf_columns(self) -> None:
        ...
        result = _walk_forward_summary(row)
        expected_keys = {
            "walk_forward_id", "search_type", "search_spec", "select_metric",
            "maximize", "refit", "status", "total_windows", "completed_windows",
            "failed_windows", "mean_validation_metric", "created_at",
