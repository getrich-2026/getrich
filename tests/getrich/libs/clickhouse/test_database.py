"""Tests for ``getrich.libs.clickhouse.database.ClickHouseClient``.
...
def _make_client() -> tuple[ClickHouseClient, _FakeClient]:
    ...
    fake = _FakeClient()
    with patch(
        "getrich.libs.clickhouse.database.clickhouse_connect.get_client",
        return_value=fake,
    ):
...
def test_connect_pings_existing_connection() -> None:
    ...
    with patch("getrich.libs.clickhouse.database.clickhouse_connect.get_client") as factory:
