...
from getrich_data.ingest.tushare.importers.reference import parse_date
from getrich_data.ingest.tushare.symbols import split_ts_code
from getrich_data.raw.tushare.fetchers.bars import month_range
...
def _stock_importer(tmp_raw_root):
    ...
    from getrich_data.ingest.tushare.importers.bars import StockBars1dImporter
...
def _daily_df():
    return pd.DataFrame(
        [{"ts_code": "600000.SH", "trade_date": "20240102", "open": 10.0, "high": 10.8,
          "low": 9.9, "close": 10.5, "pre_close": 9.8, "vol": 1000.0, "amount": 10500.0}]
    )
...
def test_stock_units_and_adj_factor(tmp_raw_root, monkeypatch):
    ...
    _write(tmp_raw_root, "adj_factor", pd.DataFrame(
        [{"ts_code": "600000.SH", "trade_date": "20240102", "adj_factor": 1.25}]))
    _write(tmp_raw_root, "stk_limit", pd.DataFrame(
        [{"ts_code": "600000.SH", "trade_date": "20240102", "pre_close": 9.8,
...
def test_pre_close_mismatch_drops_limits(tmp_raw_root, monkeypatch):
    ...
    _write(tmp_raw_root, "stk_limit", pd.DataFrame(
        [{"ts_code": "600000.SH", "trade_date": "20240102", "pre_close": 5.0,
          "up_limit": 5.5, "down_limit": 4.5}]))
...
def test_trading_status_from_suspensions(tmp_raw_root, monkeypatch):
    ...
    daily = pd.DataFrame(
        [{"ts_code": c, "trade_date": "20240102", "open": 10.0, "high": 10.8,
          "low": 9.9, "close": 10.5, "pre_close": 9.8, "vol": 1.0, "amount": 1.0}
    ...
    _write(tmp_raw_root, "adj_factor", pd.DataFrame(
        [{"ts_code": c, "trade_date": "20240102", "adj_factor": 1.0}
         for c in ("600000.SH", "000001.SZ", "600004.SH")]))
    _write(tmp_raw_root, "suspend_d", pd.DataFrame(
    ...
    imp = _stock_importer(tmp_raw_root)
    monkeypatch.setattr(
        imp, "_id_map", lambda: {"600000.SH": 1, "000001.SZ": 2, "600004.SH": 3})
...
def test_future_units(tmp_raw_root, monkeypatch):
    ...
    _write(tmp_raw_root, "fut_daily", pd.DataFrame(
        [{"ts_code": "CU2401.SHF", "trade_date": "20240102", "pre_close": 68000.0,
          "pre_settle": 68100.0, "open": 68200.0, "high": 68900.0, "low": 68000.0,
          "close": 68500.0, "settle": 68400.0, "vol": 5000.0, "amount": 34250.0,
          "oi": 12000.0}]))
...
def test_index_no_adj_factor_no_limits(tmp_raw_root, monkeypatch):
    ...
    _write(tmp_raw_root, "index_daily", pd.DataFrame(
        [{"ts_code": "000300.SH", "trade_date": "20240102", "open": 3400.0,
          "high": 3450.0, "low": 3380.0, "close": 3420.0, "pre_close": 3390.0,
