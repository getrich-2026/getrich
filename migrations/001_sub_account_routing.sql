...
-- `SET search_path=frontend`; the table is queried unprefixed, so it
...
CREATE TABLE IF NOT EXISTS frontend.strategy_sub_account_mapping (
...
COMMENT ON TABLE frontend.strategy_sub_account_mapping IS
...
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'frontend' AND table_name = 'live_positions'
    ) THEN
        ALTER TABLE frontend.live_positions
            ADD COLUMN IF NOT EXISTS account_id INT DEFAULT 1;

        COMMENT ON COLUMN frontend.live_positions.account_id IS
