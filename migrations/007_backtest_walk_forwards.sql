...
CREATE TABLE IF NOT EXISTS frontend.backtest_walk_forwards (
...
COMMENT ON TABLE frontend.backtest_walk_forwards IS
    'Walk-forward study metadata and summary. One row per persisted walk-forward run.';
COMMENT ON COLUMN frontend.backtest_walk_forwards.search_spec IS
    'Search specification serialized as JSONB. Decimal values encoded as strings.';
COMMENT ON COLUMN frontend.backtest_walk_forwards.summary_json IS
...
CREATE INDEX IF NOT EXISTS idx_backtest_walk_forwards_created
    ON frontend.backtest_walk_forwards(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_backtest_walk_forwards_status_created
    ON frontend.backtest_walk_forwards(status, created_at DESC);

CREATE TABLE IF NOT EXISTS frontend.backtest_walk_forward_windows (
...
COMMENT ON TABLE frontend.backtest_walk_forward_windows IS
    'Per-window walk-forward train sweep and validation run linkage/summaries.';
COMMENT ON COLUMN frontend.backtest_walk_forward_windows.best_params IS
    'Selected best train parameters serialized as JSONB. Decimal values encoded as strings.';
COMMENT ON COLUMN frontend.backtest_walk_forward_windows.validation_metrics_json IS
...
CREATE INDEX IF NOT EXISTS idx_backtest_walk_forward_windows_walk_index
    ON frontend.backtest_walk_forward_windows(walk_forward_id, window_index);
CREATE INDEX IF NOT EXISTS idx_backtest_walk_forward_windows_walk_status
    ON frontend.backtest_walk_forward_windows(walk_forward_id, status);
CREATE INDEX IF NOT EXISTS idx_backtest_walk_forward_windows_train_sweep
    ON frontend.backtest_walk_forward_windows(train_sweep_id);
CREATE INDEX IF NOT EXISTS idx_backtest_walk_forward_windows_validation_run
    ON frontend.backtest_walk_forward_windows(validation_run_id);
