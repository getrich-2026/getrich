...
--
-- Three nullable NUMERIC columns on ``frontend.backtest_jobs`` that let a
...
COMMENT ON COLUMN frontend.backtest_jobs.retry_base_seconds IS
    'Per-job retry base interval (seconds). NULL = use runner default.';
COMMENT ON COLUMN frontend.backtest_jobs.retry_cap_seconds IS
    'Per-job retry cap interval (seconds). NULL = use runner default.';
COMMENT ON COLUMN frontend.backtest_jobs.retry_jitter_pct IS
