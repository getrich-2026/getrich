...
## D-059 前端诊断区块状态与计算缓存分离；零权重检查先于过滤结果

表接口 v0.3.0 以 `SectionResult` 区分整节 ready/unavailable/failed，旧缓存里 B/C/D 的
null 不能直接作为网页响应。数值和报告由同一服务端组装器生成，失败方案保留输入与安全错误；
覆盖率阈值在请求级应用后再生成画像和盲点，不修改跨用户共享的计算载体。

实现零权重过滤时，曾把“已解析权重总和为零”的校验放在过滤后的非空分支内，
已有回归测试发现它不会再执行。先检查原始解析集合的权重，再排除零权重计算项；
同时保留原始输入回显，避免把零权重行算进有效持仓数。

新 API 的网络重试由 UUIDv4 幂等键识别，新键或无键创建新快照；不能继续按相同请求体
永久复用 snapshot。复用现有 request_hash 列存命名空间与幂等键的散列，原始请求体单独
比对以拒绝同键异体，事务锁覆盖匿名用户；无需改表结构。后续 DDL 注释整理应反映这一语义。
