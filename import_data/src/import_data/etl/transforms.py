...
def get_symbol_type(symbol: str) -> str:
    ...
    # 简单的期货/期权判断
    if any(
        s.endswith(suffix) for suffix in [".CCFX", ".XSGE", ".XDCE", ".XZCE", ".XINE"]
    ):
        if "-" in s:
            return "option"
        return "future"
...
def convert_date_value(
    ...
    try:
        ...
        if warn_on_error:
            log.warning(
                f"Cannot convert value to date: {val} (type={type(val).__name__})"
            )
        return default
...
def clean_date_columns(
    ...
    if date_columns is None:
        date_columns = [
            c
            for c in df.columns
            if "date" in c.lower() and "updated_at" not in c.lower()
        ]
    ...
    for col in date_columns:
        ...
        df[col] = df[col].apply(
            lambda x: (
                None
                if (isinstance(x, str) and x.strip() in INVALID_DATE_STRINGS)
                else x
            )
        )
...
def clean_dataframe_for_clickhouse(
    ...
    df = clean_date_columns(
        df, date_columns=date_columns, default=default_date, inplace=True
    )
...
if __name__ == "__main__":
    ...
    # 验证
    assert result_df["adj_factor"].iloc[0] < 1.0, (
        "adj_factor before ex-date should be < 1"
    )
    assert abs(result_df["adj_factor"].iloc[4] - 1.0) < 0.001, (
        "Latest adj_factor should be 1.0"
    )
