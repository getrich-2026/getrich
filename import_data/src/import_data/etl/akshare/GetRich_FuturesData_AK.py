...
def get_futures_contract_info(
    exchangename: str, date: str | None = None
) -> pd.DataFrame:
    """
    date = '20250731'
...
def get_futures_realtime(subscribe_str: str, market: str, adjust: str) -> pd.DataFrame:
    ...
    df: pd.DataFrame = ak.futures_zh_spot(
        symbol=subscribe_str, market=market, adjust=adjust
    )
    return df
...
def get_futures_hist_exchange(
    ...
    variety	str	品种
    """
    df: pd.DataFrame = ak.get_futures_daily(
...
# 27. 获取期货连续合约数据-新浪
def get_futures_continuous_sina(
    ...
    动态结算价	int64
    """
    df: pd.DataFrame = ak.futures_main_sina(
