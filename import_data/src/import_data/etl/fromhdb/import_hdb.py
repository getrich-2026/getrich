...
from import_data.etl.fromrq import init_rq
...
def _read_hdb_to_df(
    ...
    if (begin is None) and (end is None):
        ...
        task = hdb_file.open_read_task(
            begin_time=begin, symbols=symbols, types=[data_type]
        )
    else:
        task = hdb_file.open_read_task(
...
def _process_data_to_df(
    data_type_def: Any, data: np.ndarray, items: np.ndarray
) -> pd.DataFrame:
        ...
        if settings.ricequant.enabled:
            ...
            try:
                ...
                for original, converted in zip(
                    missing_symbols, converted_list, strict=True
                ):
                    mapping[original] = converted
            except Exception as e:
                log.warning(
                    f"Failed to convert symbols via RiceQuant: {e}. Using original."
                )
            ...
            try:
                ...
                log.warning(
                    f"Failed to convert symbols via convert_symbol: {e}. Using original."
                )
                for s in missing_symbols:
                    mapping[s] = s
...
def read_day_bar_from_local(
    ...
    if hdb is None:
        raise RuntimeError(
            "HDB module is not initialized (check settings or install hdb)"
        )
    db = hdb.DB(db_path)
...
def prepare_min_bar_for_db(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "date" in processed_df.columns and "time" in processed_df.columns:
        ...
        processed_df["bar_time"] = pd.to_datetime(
