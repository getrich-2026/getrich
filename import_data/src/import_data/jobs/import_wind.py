...
def _import_single_file(
    client: ClickHouseClient, file_path: Path, db_name: str
) -> None:
    """
    导入单个文件的核心逻辑。
...
def import_wind_data_from_parquet(
    data_dir: str = r"D:\data\wind", db_name: str = "wind"
) -> None:
    """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    # 最后一次尝试也失败
                    log.error(
                        f"Failed to import {file_path.name} after {max_retries} attempts."
                    )
