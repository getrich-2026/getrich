...
from import_data.etl.fromrq import export_all_instruments, init_rq
...
def test_config() -> None:
    ...
    if settings.ricequant.api_key:
        ...
        logger.info(
            "API Key: %s...%s (Length: %d)",
            prefix,
            suffix,
            len(settings.ricequant.api_key),
        )
...
def test_query() -> None:
    ...
    df = cast(
        pd.DataFrame,
        rq.is_suspended(symbol, start_date="20240110", end_date="20240120"),
    )
