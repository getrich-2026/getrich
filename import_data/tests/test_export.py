...
import os
from import_data.db.clickhouse import ClickHouseConnectionPool
from import_data.db.tables import MinBarTable


def export_cs_min_bars(
    ...
    for year, _, start_date, end_date, period_label in periods:
        ...
        if not df.empty:
            ...
            output_path = os.path.join(
                output_dir, f"min_bars_CS_{year}_{period_label}.parquet"
            )
            df.to_parquet(output_path)
            print(
                f"Exported {len(df)} records for {year} {period_label} to {output_path}"
            )
        else:
...
def export_indices_futures_min_bars(
    ...
    for year in years:
        ...
        if not df.empty:
            os.makedirs(output_dir, exist_ok=True)
            output_path = os.path.join(
                output_dir, f"min_bars_Indices_Futures_{year}.parquet"
            )
            df.to_parquet(output_path)
